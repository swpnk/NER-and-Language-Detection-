...
from azure_api import read_results, get_create_json_pdf
...
def read_pdf_to_image(filepath):
    ...
    images_from_path = convert_from_path(filepath, last_page=num_of_pages,
                                         first_page=0)  # Convert the pdf to a image array for all the pages
    return images_from_path


def language_detect(open_cv_image, page, sentences, files, language_list, number):
    ...
    text = ""
    text = text + " " + sentences[number][0]
    # print(sentences[number][1][idx])
    # print(len(sentences[number][1][idx]))
    ...
    try:
        ...
        cv2.putText(open_cv_image,str(language), (1500,100), cv2.FONT_HERSHEY_SIMPLEX, 2 , (255,0,0), 5) #Print on the screen, the most suitable language of the text in the contour
    except:
        language = "error"  # If the langdetect module does not return a language for the identified text. For eg: '.','222', etc
    language_list.append([language,files[number]])
    text_and_language[language].append(text)
    return open_cv_image, text_and_language, language_list
...
def contour_detection(page,text_list,pdf_name,files, language_list, number):
    # Detect contours in a image file and draw bounding boxes on them
    images_path = "C:/Users/advai/PycharmProjects/Data/Images/" + pdf_name + "/"
    open_cv_image = cv2.imread(os.path.join(images_path,page))  # Get the PIL image file as a numpy array
    open_cv_image, text_and_language, language_probs = language_detect(open_cv_image,page,text_list,files,language_list,number)  # Call the language detect function to detect the language for the page
    return open_cv_image, text_and_language, language_probs
...
def language_detection(input_path):
    ...
    for i in range(len(text_list)):
        try:
            langs = detect_langs(text_list[i][0])
            langs_prob_list.append(langs)
        except:
            langs_prob_list.append([0])
    with multiprocessing.Pool(processes=4) as pool:
        language_list = multiprocessing.Manager().list()
        func = partial(get_contours, input_path, page_list, text_list, pdf_name, language_list)
...
def get_contours(input_path, page_list, text_list, pdf_name, language_list, number):
    file_path = input_path  # source path for the pdf
    ...
    images_path = "C:/Users/advai/PycharmProjects/Data/Images/" + csv_name + "/"
    page = sorted(os.listdir(images_path),key=len)
    open_cv_image, text_and_language, language_list = contour_detection(page[number],text_list, pdf_name, page, language_list, number)  # Call the contour_detection function to detect contours in the image and return the modified array
    page_list_temp = []
    ...
    cv2.imwrite(out_path + "/page_{}_language_detection.png".format(number+1), open_cv_image)
    for language in text_and_language.keys():
        if out_path.split()[-1] == '/':
            dest_path = out_path + 'page_{}/{}/'.format(number+1,
                                                        language)  # Destination path for the sentences based on the output label that they are mapped to. The 2 variables appended to the path are, 1. The csv_file name to create a folder for every csv_file ,and 2. The output label that a sentence is classified into.
        else:
            dest_path = out_path + '/page_{}/{}/'.format(number+1, language)
        ...
        for paragraph in text_and_language[language]:
            try:
                with open(dest_path + "sentences.txt",
                          "w+") as file:  # If already a sentences.txt file exists, replace it by the new predictions
                    file.write(
                        paragraph)  # Write the output sentences into a txt file in the directories that they belong to
                ...
                with open(dest_path + "sentences.txt",
                          "w+") as file:  # Create a new file called sentences.txt in the folder
                    file.write(
                        paragraph)  # Write the output sentences into a txt file in the directories that they belong to
                    file.write("\n")  # Separate every sentence with a new line
    gc.collect()
...
if __name__ == "__main__":  # main function only called when the script is running and not while importing modules
    ...
    page_list = language_detection("C:/Users/advai/PyCharmProjects/Data/Pdfs/Test_PDF.pdf")
    # pdf_name = "C:/Users/advai/PyCharmProjects/Data/Pdfs/TickBox_1.pdf".split("/")[-1].split(".pdf")[0]
    # words, sentences = read_results(pdf_name)
    # print(len(sentences))
    # words,sentences = read_results("TickBox_1")
    # text = ""
    # for i in range(len(sentences[0][1])):
    #     text = text + " "+sentences[0][1][i][0]
    #     # text = " ".append(sentences[0][1][i][0])
    # print(text)
    # langugae = langdetect.detect(text)
    # print(langugae)
